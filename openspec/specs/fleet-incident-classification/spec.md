# fleet-incident-classification Specification

## Purpose
Asking a model what KIND of failure something was, and nothing more.

The classifier classifies; code chooses the workflow. A one-token answer never
names a tier, never provisions, and never touches region, quota or budget. That
split is measured rather than squeamish: on four short, distinct triage labels
this transport shows about 10% order instability, which is usable for "which
workflow should run" and not for "should we rent the expensive thing".

Three refusals hold it together, each because the alternative is worse than no
answer. Required evidence REFUSES rather than trims, because a trimmed packet
produces an answer indistinguishable from an informed one. An invariant that
rejects the winner leaves NO selection, because taking the runner-up is code
overruling a model with a guess. And an unreachable classifier resolves to a
human block with no provisioning, because it runs on the fleet's own capacity
and bursting to restore it is a spending loop with an outage for a trigger.
