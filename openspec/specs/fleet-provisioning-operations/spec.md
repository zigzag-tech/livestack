# fleet-provisioning-operations Specification

## Purpose
Renting a machine, and being able to prove whether you did.

Everything here serves one sentence: **one logical operation results in at most
one billed create.** A provisioning record is written, with its idempotency key,
before the provider is called — so a process that dies in between comes back to
that row and RECONCILES it by asking, never by retrying. "The provider did not
answer" and "the provider said no" are kept apart, because a control plane that
cannot tell them apart will eventually choose the expensive reading of silence.

The same discipline runs the other way. A worker is released only after a drain
claim, an empty node and a final authoritative re-check: `schedule()`'s
`Deprovision` is a proposal computed from a view that is already seconds old,
and a view is never a proof.
