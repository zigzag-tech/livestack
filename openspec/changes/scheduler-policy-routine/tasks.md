...
- [ ] 3.2 In `/fleet/admit` (`hostd.py`) and `fleet_admit.admit()`:
  ...
  3. Have `PolicyRuntime` write `plan.decisions[job.id]` as a J§6.2 record to the policy stream, with `self_traffic` computed from `LIVESTACK_POLICY_SELF_PRINCIPALS` and the authenticated principal. Put the pointer on the admit ledger record.
