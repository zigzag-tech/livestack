...
impl DefGraph {
    ...
    pub fn assign_alias(
        ...
        let root_spec_id = unique_spec_identifier(root_spec_name, unique_spec_label);
        ...
        let spec_id = unique_spec_identifier(spec_name, unique_spec_label);
        ...
        let alias_node_id = self.node_indices.get(&alias_id).copied().unwrap_or_else(|| {
            self.ensure_node(
                &alias_id,
                DefGraphNode {
