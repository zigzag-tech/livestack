"""LivestackCoordinator — the lease-driven implementation of the Coordinator seam.
...
from .manager import ResidencyPolicy
...
class Coordinator(Protocol):
    """Policy seam for residency (formerly ``polycore.Coordinator``). Methods run on
    the GPU executor thread, with the manager's ``_guard`` held by the caller.
    Implementations drive the manager's ``_load``/``_evict`` primitives; they never
...
class LocalCoordinator:
    """The in-process residency discipline (formerly ``polycore.LocalCoordinator``).
