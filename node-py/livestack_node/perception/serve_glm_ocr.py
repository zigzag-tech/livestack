"""Managed Harmony worker for GLM-OCR text recognition."""
...
from .admission import GLM_OCR_UNIT, broker_admission_payload
from .contract import PerceptionContractError
from .glm_ocr import GlmOcrAdapter, GlmOcrRuntime
from .serve import _post_json
from .service import PerceptionService, build_app


def create_app():
    host_id = os.environ.get("HARMONY_PERCEPTION_HOST_ID", "xc-tower-glm-ocr")
    port = int(os.environ.get("HARMONY_PERCEPTION_PORT", "8203"))
    ...
    snapshot = os.environ.get("GLM_OCR_SNAPSHOT")
    if not snapshot:
        raise RuntimeError("GLM_OCR_SNAPSHOT is required")
    model_id = os.environ.get("GLM_OCR_MODEL", "zai-org/GLM-OCR")
    revision = os.environ.get("GLM_OCR_REVISION", "unknown")
    footprint = int(os.environ.get("GLM_OCR_RESIDENT_BYTES", str(6 * 1024**3)))
    busy = counting()
    lock = threading.Lock()
    holder: dict[str, object] = {}

    def load_runtime():
        runtime = GlmOcrRuntime(snapshot)
        runtime._load()
    ...
    units = {GLM_OCR_UNIT: ManagedUnit(
        GLM_OCR_UNIT, load_runtime, free_runtime, footprint=footprint,
    ...
    def generate(path, prompt, max_tokens, control=None):
        with busy, lock:
            return holder["manager"].run(
                GLM_OCR_UNIT, lambda runtime: runtime(path, prompt, max_tokens, control))
...
def main():
    ...
    uvicorn.run(create_app(), host="0.0.0.0",
                port=int(os.environ.get("HARMONY_PERCEPTION_PORT", "8203")), workers=1)
