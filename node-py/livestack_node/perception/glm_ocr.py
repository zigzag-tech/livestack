"""GLM-OCR provider for ``jingway.perception.v1`` text recognition."""
...
class GlmOcrAdapter:
    ...
    def infer(self, request: dict, *, grant: dict, control=None) -> dict:
        if request["task"]["type"] != "text_recognition":
            raise PerceptionContractError("unsupported", "GLM-OCR serves text_recognition", 422)
        prompt=request["task"].get("prompt") or "Text Recognition:"
        ...
        for image in request["images"]:
            ...
            raw.append(text);observations.append({"id":f"ocr-{len(observations)+1}","imageId":image["id"],"text":text,"attributes":{"reader":"glm-ocr"}})
        ...
        joined="\n\n".join(raw)
        return {"schemaVersion":"jingway.perception.v1","requestId":request["requestId"],"outcome":"ok" if observations else "empty","observations":observations,"unprocessed":[],"truncated":False,"rawOutputSha256":hashlib.sha256(joined.encode()).hexdigest(),"execution":{"backend":"cuda","device":grant.get("device") or self.device,"implementation":"glm-ocr-transformers-v1","model":self.model,"modelRevision":self.revision,"precision":"bf16","preprocessingRevision":"glm-ocr-chat-template-v1","queueMs":float(grant.get("queue_ms",0)),**metrics}}
...
class GlmOcrRuntime:
    ...
    def _load(self):
        ...
        self._model=GlmOcrForConditionalGeneration.from_pretrained(self.snapshot,dtype=torch.bfloat16,local_files_only=True).to("cuda:0").eval();self._load_ms=(time.perf_counter()-started)*1000
